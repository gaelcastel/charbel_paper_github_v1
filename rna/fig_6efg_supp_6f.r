...
xi_fraction_vector_charbel=xi_fraction_vector[rn(sample_annot)]

df_plot=NULL
df_plot$sample=rn(sample_annot)
df_plot$cell_type=sample_annot$cell_type
df_plot$group=sample_annot$group
df_plot$cell_type_group=paste(sep="_",sample_annot$cell_type,sample_annot$group)
df_plot$xi_fraction=xi_fraction_vector_charbel
df_plot$sample_name=sample_annot$Sample_Name
...
df_plot$cell_type_group=factor(df_plot$cell_type_group,levels = c("pxgl_siScr","pxgl_siSPEN_1","pxgl_siSPEN_2",
                                                                  "primed_siScr","primed_siSPEN_1","primed_siSPEN_2")) # to place in appropriate order on the plot



ggplot(data=df_plot,aes(x=cell_type_group,y=xi_fraction,fill=cell_type_group))+
...
for (sample in rn(sample_annot)){
  ...
  sample_list_gene_fraction[[sample]]$sample_name=rep(sample_annot[sample,'Sample_Name'],nrow(sample_list_gene_fraction[[sample]]))
  sample_list_gene_fraction[[sample]]$cell_type_group=rep(paste(sep="_",sample_annot[sample,'cell_type'],sample_annot[sample,'group']),nrow(sample_list_gene_fraction[[sample]]))
  sample_list_gene_fraction[[sample]]$cell_type=rep(sample_annot[sample,'cell_type'],nrow(sample_list_gene_fraction[[sample]]))
}


sample_list_gene_fraction=do.call('rbind',sample_list_gene_fraction)

sample_list_gene_fraction$cell_type_group=factor(sample_list_gene_fraction$cell_type_group,levels = c("pxgl_siScr","pxgl_siSPEN_1","pxgl_siSPEN_2",
