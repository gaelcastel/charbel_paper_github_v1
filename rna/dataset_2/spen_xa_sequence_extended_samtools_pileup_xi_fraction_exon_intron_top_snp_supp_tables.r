...
for(sample in names(sample_list)){
  ...
  write.tsv(df_to_be_written,file=paste0("",paste(sep = "_",sample,"_chrX_snp_table.tsv")),row.names = F)
}
...
for(sample in names(sample_list)){
  df_to_be_written=sample_list[[sample]][,c("chrom","pos","dp","allele_maj","allele_maj_num","allele_min","allele_min_num","allelism")]
  write.tsv(df_to_be_written,file=paste0("",paste(sep = "_",sample,"_chr7_snp_table.tsv")),row.names = F)
}
