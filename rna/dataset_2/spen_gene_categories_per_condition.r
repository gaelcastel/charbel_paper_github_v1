...
spen_ctl_ttt_1_snp$xi_mean_fraction_pxgl_siScr[spen_ctl_ttt_1_snp$xi_mean_fraction_pxgl_siScr==0]<-0.01
spen_ctl_ttt_1_snp$fc=spen_ctl_ttt_1_snp$xi_mean_fraction_pxgl_siScr/spen_ctl_ttt_1_snp$xi_mean_fraction_pxgl_siScr
...
spen_ctl_ttt_1_snp$category<-apply(spen_ctl_ttt_1_snp,1,function(x){
  ...
  if(as.numeric(x['xi_mean_fraction_pxgl_siScr'])<=0.25 & as.numeric(x['xi_mean_fraction_pxgl_siScr'])<=0.25 & data.table::between(as.numeric(x['fc']),1/1.2,1.2)){return('mono_mono')}# I checked, no one > 0.75
  else if(as.numeric(x['xi_mean_fraction_pxgl_siScr'])<=0.25 & as.numeric(x['xi_mean_fraction_pxgl_siScr'])<=0.25 & as.numeric(x['fc'])>=1.2){return('mono_mono_up')}
  else if(as.numeric(x['xi_mean_fraction_pxgl_siScr'])<=0.25 & as.numeric(x['xi_mean_fraction_pxgl_siScr'])<=0.25 & as.numeric(x['fc'])<=1/1.2){return('mono_mono_down')}
  else if(as.numeric(x['xi_mean_fraction_pxgl_siScr'])>=0.25 & as.numeric(x['xi_mean_fraction_pxgl_siScr'])>=0.25 & data.table::between(as.numeric(x['fc']),1/1.2,1.2)){return('bi_bi')}
  else if(as.numeric(x['xi_mean_fraction_pxgl_siScr'])>=0.25 & as.numeric(x['xi_mean_fraction_pxgl_siScr'])>=0.25 & as.numeric(x['fc'])>=1.2){return('bi_bi_up')}
  else if(as.numeric(x['xi_mean_fraction_pxgl_siScr'])>=0.25 & as.numeric(x['xi_mean_fraction_pxgl_siScr'])>=0.25 & as.numeric(x['fc'])<=1/1.2){return('bi_bi_down')}
  else if(as.numeric(x['xi_mean_fraction_pxgl_siScr'])<=0.25 & as.numeric(x['xi_mean_fraction_pxgl_siScr'])>=0.25){return('mono_bi_up')}
  else if(as.numeric(x['xi_mean_fraction_pxgl_siScr'])>=0.25 & as.numeric(x['xi_mean_fraction_pxgl_siScr'])<=0.25){return('bi_mono_down')}
