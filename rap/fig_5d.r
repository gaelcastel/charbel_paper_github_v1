...
df_list=lapply(matrices,function(x){
  ...
  rownames(df)=df$V4
  df=df[,7:ncol(df)]
  pxgl=rowSums(df[,1:200])
  primed=rowSums(df[,201:400])

  return(data.frame(pxgl,primed))
...
df$condition[df$condition=="pxgl"]<-"naive"
...
df %>%
  ...
  # stat_compare_means(aes(x=category, y=norm_count),vjust=0.1,label="p.signif",size=3, method="t.test", paired=FALSE) +
  ggtitle("RAP-Seq XIST sensitive/unsensitive gene categories\nWT Naive vs Primed - TSS 5kb") -> p

res.stat_condition <- df %>%
  group_by(category) %>%
  ...
  add_significance("p") %>%
  ungroup()%>%
  add_xy_position(x = "category", fun = "max",formula = norm_count ~ condition, scales = c("free_y")) %>%
  ungroup()


res.stat_category <- df %>%
  group_by(condition) %>%
  ...
  add_significance("p") %>%
  ungroup()%>%
  add_xy_position(x = "category", group = "condition", fun = "max",formula = norm_count ~ category,step.increase = 0.12) %>%
  ungroup()


#pdf("rap_gene_categories_naive_primed_tss_5kb.pdf",width=5,height=7)

print(p 
      
      +
        stat_pvalue_manual(
          res.stat_condition, label = "p.signif"
        )
      +
        stat_pvalue_manual(
          res.stat_category, label = "p.signif"
        )
...
df %>%
  ...
  # geom_point(aes(x=category, y=norm_count, fill=condition),size=0.25,position=position_jitterdodge(jitter.width=0,dodge.width=0.8),alpha=0.8)+
  facet_wrap(~condition, nrow=2, ncol=3,scales = 'free')+
  theme_bw()+
  theme(strip.text.x = element_text( face = "bold.italic", size = 10))+
  guides(fill=guide_legend(ncol=1)) +
  theme(axis.line.x = element_blank(),
  ...
  # stat_boxplot(aes(x=category, y=tpm),geom='errorbar', linetype=1, width=0.8, size=0.25)+
  stat_compare_means(aes(x=category, y=norm_count),vjust=0.1,label="p.signif",size=3, method="t.test", paired=FALSE) +
  ggtitle("RAP-Seq XIST sensitive/unsensitive gene categories\nin all cell types - TSS 5kb")

#ggsave("rap_gene_categories_all_cell_types_tss_5kb.pdf",width=5,height=7) #fig 5d
