...
bi_bi_up=read.table("D:/rougeulle_lab_ordi_sept_2023/charbel_paper/rna/charbel_paper_ifb/embryo_pseudobulk/bi_bi_up.gtf", header=F, sep="\t")
...
mono_bi_up=read.table("D:/rougeulle_lab_ordi_sept_2023/charbel_paper/rna/charbel_paper_ifb/embryo_pseudobulk/mono_bi_up.gtf", header=F, sep="\t")
...
bi_bi=read.table("D:/rougeulle_lab_ordi_sept_2023/charbel_paper/rna/charbel_paper_ifb/embryo_pseudobulk/bi_bi.gtf", header=F, sep="\t")
...
pdf("D:/charbel_december_2023/gene_categories_expression_in_stem_cells_violin_plots_wt.pdf",height = 8,width = 5)
...
pdf("D:/charbel_december_2023/gene_categories_expression_in_stem_cells_violin_plots_all_cell_types_pval.pdf",height = 6,width = 8)
...
dataForViolinBU=reshape2::melt(data.frame(t(mmat_mtx_cpm[gene_cat$gene,samples]),sample=sample_annot[samples,"Sample_Name"],
                                          dataset=sample_annot[samples,"dataset"],
                                          cell_type_condition_dataset=paste(sep="_",sample_annot[samples,"cell_type"],
                                                                            sample_annot[samples,"dox_ko"],
                                                                            sample_annot[samples,"dataset"]),
                                          x_median=stat_x[samples],
                                          a_median=stat_a[samples],check.names = F),
                               value.name="cpm",variable.name="gene",id.vars=c("sample","dataset","cell_type_condition_dataset","x_median","a_median"))
...
dataForViolinBU$category[dataForViolinBU$gene%in%up_new]<-"up_new"
dataForViolinBU$category[dataForViolinBU$category=="up"]<-"up_old"
...
dataForViolinBU$category=factor(dataForViolinBU$category)

dataForViolinBU %>%
  group_by(cell_type_condition_dataset) %>%
  mutate(x_median=mean(x_median),a_median=mean(a_median)) -> dataForViolinBU

pdf("D:/charbel_december_2023/gene_categories_violin_plots_stats_x_a_medians_new_categories.pdf",height = 6,width = 8*1.33)

ggplot(dataForViolinBU,aes(x=cell_type_condition_dataset,y=cpm))+
  geom_violin(aes(fill=cell_type_condition_dataset),scale = "width")+
  geom_boxplot(aes(fill=cell_type_condition_dataset),outlier.shape = NA,width=0.3,position=position_dodge(.9))+
  theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=.3,size=8),
        panel.background = element_rect(fill = NA,colour="black"),
  ...
  ggtitle("Gene categories expression in stem cells")+
  stat_compare_means(comparisons = my_comparisons, method="t.test", paired=FALSE,bracket.size = 0, label.y = 7.4,vjust = 0.1,)+
  stat_compare_means(comparisons = my_comparisons, method="t.test", paired=FALSE, label.y = 7.4, label="p.signif",vjust = 2)+
  geom_pointline(aes(y=x_median,group=dataset), colour="green", position =position_dodge(.9))+
  geom_pointline(aes(y=a_median,group=dataset), colour="red", position =position_dodge(.9))+
  scale_y_continuous(trans="pseudo_log",breaks=c(0,100,500,1000,2000))+
  facet_wrap(~category)
